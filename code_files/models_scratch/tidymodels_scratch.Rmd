...
This file has been a continuous progress of applying tidymodels to the models. This document includes errors and failed attempts, but tracks the whole work process of figuring out the best methods of machine learning in our modeling.

In the last section, I used the multilevelmod extension package to add family as a random effect. The cleaned process of successes will be continuously transferred to the tidymodels_clean.Rmd file where I will run the models with species and family as both fixed and random effects.
...
* ML Attempt #2 - FAILED
* ML Attempt #3 - SUCCESS
* Adding random effects with multilevel mod
...
This was Luna's and my really short ChatGPT attempt to figure out how to split the data and leave out species. I do not want to delete this because I am not sure if we ended up using any of this.
...
In this section, I reran the models using family instead of species, and to include them as a random effect, I found a parsnip extention package called **multilevelmod** which allows us to use bayesian regression with random effects.

*This is Luna's code*

Luna was able to add family into the data frame so we can use that as a predictor instead of species for generalizability.
...
This is also code that Luna was able to complete.
...
```{r}
# load multilevelmod package
library(multilevelmod)

# create model specusing stan_glmer
...
# workflow with family as a random effect
lmer_wflow <- 
  workflow() %>% 
...
# workflow with species as a random effect
species_wf <-  workflow() %>% 
...
# train using the multilevelmod workflows ----
...
```{r}
# predict using multilevelmod workflow ----
# family
...
Now that we have almost finished the ML model, we need to figure out which combination of predictors performs the best.
