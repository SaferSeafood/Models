...
All previous code processes in tidymodels can be found in tidymodels_scratch.Rmd. This file is for running the models using family and species as random effects in tidymodels.
...
test_predict_fam <- predict(brm_fam_fit2, fam_test) %>% #get prediction probabilities for test data
  bind_cols(fam_test)

test_predict_species <- predict(brm_species_fit, species_test) %>% 
  bind_cols(species_test)

predict_species_int <- predict(brm_species_fit, species_test, type = "pred_int") %>% 
  bind_cols(species_test)
```
...
### export model
```{r}
saveRDS(brm_species_fit, file = here::here("data", "data_outputs", "speciesRandEffect.rda"))
```
