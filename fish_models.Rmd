...
# Add transformed columns (TotalDDT.lipid is already normalized by Lipid for fish)
fish_reg$TotalDDT.trans.non = log(fish_reg$TotalDDT + 1)
fish_reg$TotalDDT.sed.trans = log(fish_reg$TotalDDT.sed + 1)
...
fish_reg %>% 
  ...
  ggplot(mapping=aes(x=TotalDDT.sed.trans, # ddt concentration in sediment 
                     y=TotalDDT.trans.non, # ddt concentration in lipids
...
lm(TotalDDT.trans.non ~ TotalDDT.sed.trans + trophic_category, data = fish_reg) %>% 
  summary()
